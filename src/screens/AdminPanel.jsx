...
export default function AdminPanel({ leads, stats, storage, onUnlock, onRefresh, onExport, onClear, onBack }) {
  ...
  return (
    <div className="screen bg-gray-950">
      {/* Header */}
      <div className="shrink-0 flex flex-col gap-3 px-4 sm:px-6 lg:px-8 py-4 bg-gray-900 border-b border-gray-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex min-w-0 items-center gap-2">
            <img src="/logo-continental.png" alt="" className="h-9 w-auto logo-white opacity-70" />
            <span className="truncate text-white font-bold text-sm sm:text-base">Panel de administración — Feria Madera</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-end gap-2">
          {!locked && (
            <button onClick={() => onRefresh()} disabled={storage?.loading}
              className="btn bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-white font-bold text-sm px-3 sm:px-4 py-2 rounded-lg min-h-0">
              ↻ Actualizar
            </button>
          )}
          <button onClick={onExport} disabled={leads.length === 0}
            className="btn bg-green-800 hover:bg-green-700 disabled:opacity-40 text-white font-bold text-sm px-3 sm:px-4 py-2 rounded-lg min-h-0">
            📥 Exportar CSV
          </button>
          {!confirmClear
            ? <button onClick={() => setConfirmClear(true)} disabled={leads.length === 0}
                className="btn bg-red-900 hover:bg-red-800 disabled:opacity-40 text-white font-bold text-sm px-3 sm:px-4 py-2 rounded-lg min-h-0">
                🗑️ Limpiar
              </button>
      ...
      {/* Stats bar */}
      {!locked && <div className="shrink-0 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 bg-gray-900 border-b border-gray-800">
        {[
          ...
          <div key={s.label} className="min-w-0 text-center py-3 px-2 border-r border-b border-gray-800 xl:border-b-0 last:border-r-0">
            <div className={`text-2xl sm:text-3xl font-black ${s.color}`}>{s.val}</div>
            <div className="truncate text-gray-500 text-xs mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>}

      {/* Tab nav */}
      {!locked && <div className="shrink-0 flex flex-wrap gap-1 px-4 sm:px-8 py-3 bg-gray-900 border-b border-gray-800">
      ...
      {/* Content */}
      {!locked && <div className="flex-1 min-h-0 scroll">
        {tab === 'leads' && (
          leads.length === 0
