...
// ── Página (react-pageflip requiere forwardRef) ────────────────────────────
const Page = forwardRef(({ src, pageNum }, ref) => (
  <div ref={ref} style={{ userSelect: 'none', overflow: 'hidden', background: '#fff' }}>
    {src ? (
      <img
        src={src}
        alt={`Página ${pageNum}`}
...
export default function BrochureScreen({ onClose }) {
  ...
  async function loadPDF() {
    try {
      ...
      const targetH   = 800                           // altura de renderizado interno
  ...
  // Mide el contenedor disponible y calcula cuánto hay que escalar el flipbook
  ...
  const recalcScale = useCallback(() => {
    ...
    // El flipbook en modo libro ocupa 2 páginas de ancho
    const flipNativeW = pageSize.w * 2
    const flipNativeH = pageSize.h
    const padH = 12   // espacio vertical para controles + margen
    const padW = 24   // margen lateral mínimo
    const scaleW = (availW - padW) / flipNativeW
    const scaleH = (availH - padH) / flipNativeH
    // Nunca agrandar más allá del tamaño nativo; nunca encoger más del 20%
  ...
  const displayEnd  = Math.min(currentPage + 2, totalPages)
  const pageLabel   = displayPage === displayEnd
    ? `Página ${displayPage} de ${totalPages}`
    : `Páginas ${displayPage}–${displayEnd} de ${totalPages}`

  // Dimensiones del "slot" visual que ocupa el flipbook escalado
  const slotW = pageSize.w * 2 * displayScale
  ...
  return (
    <div className="screen bg-[#181818] flex flex-col">
      ...
      {/* ── Header ── */}
      <div className="shrink-0 flex items-center justify-between px-8 py-3 border-b border-white/10 bg-black/30">
        <button
          onClick={onClose}
          className="btn flex items-center gap-2 text-white/70 hover:text-white px-4 py-2 rounded-xl hover:bg-white/10 transition-colors font-semibold text-sm"
        >
        ...
        {!loading && !error && (
          <p className="text-white/35 text-xs text-right leading-snug w-32">
            Desliza las páginas<br />o usa las flechas
          </p>
        )}
        ...
        {!loading && !error && pages.length > 0 && (
          ...
          >
            {/*
              ...
              - El div interior tiene las dimensiones NATIVAS y se escala con transform
              - transformOrigin: top left → el escalado ocurre desde la esquina superior izquierda
                que coincide exactamente con el origen del slot exterior
            */}
            <div
              style={{
                width:    slotW,
            ...
            <div className="flex items-center gap-5">
              <button onClick={goPrev}
                className="btn w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
